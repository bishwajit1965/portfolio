    ...
    <div className="relative border border-base-300 dark:border-slate-700 bg-base-200 dark:bg-slate-800 shadow-md rounded-xl p-6 flex flex-col justify-between h-full transition-transform duration-300 hover:scale-105">
      ...
      <div className="flex items-center gap-4 mb-4">
        <img
          src={photo}
          alt={name}
          className="w-16 h-16 sm:w-20 sm:h-20 rounded-full object-cover ring-2 ring-blue-500 dark:ring-amber-400 shadow-md"
        />
        <div>
      ...
      {/* Testimonial Text */}
      <p className="text-sm sm:text-base leading-relaxed text-gray-600 dark:text-gray-300 italic mb-3">
        “ {testimonial} ”
      </p>

      {/* Project Info */}
      {projectName && (
        <p className="text-xs text-gray-500 dark:text-amber-400 mb-3">
          Project: <span className="font-medium">{projectName}</span>
        </p>
      )}

      {/* Footer */}
      <div className="flex justify-between items-center pt-3 border-t border-base-200 dark:border-slate-700 text-sm sm:text-base">
        <span className="text-gray-500 dark:text-gray-400">{location}</span>
        <span className="flex items-center gap-1 font-semibold text-blue-600 dark:text-amber-400">
          ⭐ {rating}/5
        </span>
      </div>
    </div>
